...
const Dashboard = (props: DashboardProps) => {
  ...
  useEffect(() => {
    if (props.webSocket === undefined || props.peerConnection === undefined) {
      setFatalErrorDialog(true);
      setFatalErrorMessage(
        "Our services are currently unavailable. Please try logging in again."
      );
  ...
  const fetchCourses = () => {
    ...
    axios
      ...
      .then((response) => {
        const [courseArray, courseMap] = produceCourseCache(response.data);
      ...
      axios
        ...
        .then((response) => {
          ...
          response.data.courses.forEach((enrolledCourse) => {
            enrolledCourse.course.courseId = enrolledCourse.courseID;
  ...
  const onSubmitProfileChanges = () => {
    axios
      .get<any, AxiosResponse<GetUserResponse>>(
        process.env.REACT_APP_BACKEND_API_HOST + "/api/user/" + thisUser?.id,
        {
          timeout: 5000,
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("accessToken")}`,
  ...
  const onSubmitNewQuestion = (course: Course) => {
    setSnackBarMessage(
      `Successfully posted question for ${
        course.subject.courseCode.courseCode + course.subject.courseNumber
      }.`
    );
  };
  ...
  const enrollInCourse = (user: User, course: Course) => {
    ...
    axios
      .post<EnrollRequest, AxiosResponse<any>>(
        process.env.REACT_APP_BACKEND_API_HOST + "/api/enrollment/enroll",
        request,
        {
          timeout: 5000,
          headers: {
    ...
    if (Number(roleID) == 3) {
      ...
      return (
        <ListItem // Navigation: ADMIN
          disablePadding
          className={`${
            currentPage === Page.Admin // Change this
              ? "bg-primary"
              : ""
          } `}
        >
    ...
    <div className="flex flex-col w-screen h-screen bg-tertiary overflow-y-hidden">
            ...
            <div className="flex flex-col h-full w-full p-5 justify-between">
              <div className="flex flex-col mb-5">
                ...
                <Autocomplete<Course>
                  ...
                  options={cachedCourses !== undefined ? cachedCourses : []}
                />
              </div>
              <div className="flex flex-row">
                <Button
                  ...
                  onClick={() => {
                    ...
                    enrollInCourse(thisUser, selectedCourseToEnroll);
                  }}
                >
                  Enroll
                </Button>
      ...
      <div className="flex flex-row w-full h-[75px] bg-primary justify-between z-10 min-h-[75px] drop-shadow-md shadow-md">
        <div className="flex flex-row items-center">
          <IconButton
            className={`ml-5 ${
              showSideNav ? "bg-secondary shadow-black shadow-md" : ""
            }`}
            onClick={() => {
      ...
      <div className="h-full flex flex-row overflow-y-auto">
        <div
          className={`max-w-[250px] flex flex-col h-full overflow-y-auto bg-secondary ${
            showSideNav ? "slide-left-full" : "slide-right-full"
          }`}
        >
          <List disablePadding className={`${showSideNav ? "" : "hidden"}`}>
            {
            ...
            >
              ...
              >
                ...
                <p className="font-jakarta-sans text-white text-md">Home</p>
              </ListItemButton>
            </ListItem>
            <ListItem
              disablePadding
              className={`${
                currentPage === Page.Calendar ? "bg-primary" : ""
              } `}
            >
              <ListItemButton
                className="h-[75px]"
              ...
              >
                ...
                <p className="font-jakarta-sans text-white text-md">Calendar</p>
              </ListItemButton>
            </ListItem>
            <ListItem // Navigation: Message
              disablePadding
              className={`${
                currentPage === Page.Messages // Change this
                  ? "bg-primary"
                  : ""
              } `}
            >
              ...
              >
                ...
                <p className="font-jakarta-sans text-white text-md">Messages</p>
              </ListItemButton>
            </ListItem>
            ...
            {enrolledCourses.length === 0 && !isFetchingEnrolledCourses && (
              <ListItem>
            ...
            {enrolledCourses.length > 0 &&
              ...
              enrolledCourses.map((course, idx) => (
                <ListItem disablePadding key={idx}>
                  <ListItemButton
                    className={`h-[75px] flow-up-animation ${
                      selectedCoursePage ===
                      course.subject.courseCode.courseCode +
                        course.subject.courseNumber
                        ? "bg-primary"
        ...
        {currentPage === Page.Home && (
          <Home
            ...
            enrolledCourseClickCallback={onHomePageEnrolledCourseClick}
          />
        )}
